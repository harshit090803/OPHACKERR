...
# Process-local TTS cache. This removes the repeated Edge-TTS round-trip while
# the video teacher is being demonstrated. It is intentionally bounded.
...
def _get_ca_bundle() -> str:
    # Explicit project setting wins, then the standard SSL_CERT_FILE variable,
    # then certifi. This also works cleanly on Windows and Linux containers.
...
def _get_anthropic_client(timeout: float = 60.0) -> anthropic.AsyncAnthropic:
    ...
    if anthropic_client is None:
        ...
        if ANTHROPIC_INSECURE_TLS:
            # Emergency-only local debugging switch. Never enable this for the
            # hackathon deployment because it disables certificate validation.
...
async def _edge_tts_bytes(text: str, voice: str = None) -> bytes:
    ...
    # De-duplicate simultaneous requests for the same slide.
...
async def _warm_lesson_audio(lesson: dict) -> None:
    ...
    if not slides:
        return
    # Warm the first two slides synchronously. The browser asks for these first
    # and therefore gets a cache hit instead of waiting for Edge-TTS.
    first = slides[:2]
    await asyncio.gather(*[_edge_tts_bytes(s["narration"]) for s in first])
    # Warm the remaining slides in the background; this does not block lesson
    # rendering and keeps later slide transitions smooth.
...
async def get_lesson(doc_id: str):
    ...
    if isinstance(cached, dict) and cached.get("slides"):
        # A cached lesson may have been generated by the old build, so warm its
        # first slides too. Cache hits make this effectively instant after the
        # first request.
